...
use starknet::get_contract_address;
  ...
  struct Storage {
      ...
      min_withdrawal_limit: u256, // New: Minimum withdrawal limit
  }
  ...
  #[abi(embed_v0)]
    impl ThellexPOSFactoryImpl of IThellexPOSFactory<ContractState> {
        fn initialize(ref self: ContractState, treasury: ContractAddress, fee_percent: u256, tax_percent: u256, timeout: u64, min_withdrawal_limit: u256) {
            ...
            assert(min_withdrawal_limit > 0, 'Invalid min withdrawal limit');
            ...
            self.emit(Event::FactoryInitialized(FactoryInitialized {
                treasury, fee_percent, tax_percent, timeout, admin: get_caller_address()
            }));
        }

        fn create_pos(ref self: ContractState, owner: ContractAddress, pos_class_hash: ClassHash) -> ContractAddress {
            ...
            calldata.append(self.min_withdrawal_limit.read().low.into());
            calldata.append(self.min_withdrawal_limit.read().high.into());
        ...
        fn update_min_withdrawal_limit(ref self: ContractState, new_limit: u256) {
            ...
            assert(new_limit > 0, 'Invalid min withdrawal limit');
        ...
        // Other functions (add_supported_token, remove_supported_token, etc.) remain unchanged
